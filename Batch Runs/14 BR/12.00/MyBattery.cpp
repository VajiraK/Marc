...
WORD		batavg = 0;
	...
	//Initialize
	agent.Initialize((CMyStock*)this,m_curent,m_facing,stock,&m_sec_pic_loc);
	...
	for(int i=(from-1);i>=1;i--)
	{
		...
		if(m_path[i].att==PA_TELE)
		{
			...
			if(m_path[i].steps!=0)
				batavg = agent.Move(m_path[i].steps);
		}
	}

/*#ifdef _DEBUG
	BYTE s = GetStock(ST_SECURE);
	BYTE t = GetStock(ST_COL_SEC);
	if(agent.m_packh<t)
		MessageBox(NULL,"Error bat!   ","Marc",MB_OK|MB_ICONEXCLAMATION);
#endif*/

	return agent.m_bat+batavg;
}

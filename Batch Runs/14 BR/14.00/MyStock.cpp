...
BYTE in;
	...
	case MODE_COLOR://************************
		...
		//Insert new item
		m_stock.color.packs[in].free   = INVALIED;
	...
	case MODE_BLACK://************************
		...
		m_stock.black.packs[in].free   = INVALIED;
	...
	case MODE_FREE_COLOR://************************
		...
		//Select a free item
		in = m_stock.fcolor.count;
		//Add
		m_stock.fcolor.totalh += item->h;
		m_stock.fcolor.packs[in].free	= VALIED;
		m_stock.fcolor.packs[in].type   = item->type;
		m_stock.fcolor.packs[in].h	    = item->h;
		m_stock.fcolor.packs[in].ntag   = item->ntag;
		memcpy(m_stock.fcolor.packs[in].stag,item->stag,4);
		m_stock.fcolor.count++;
	}
...
BYTE i;
	...
	case MODE_BLACK://*************************
		...
		for(i=0;i<m_stock.black.count;i++)
		{
			if(m_stock.black.packs[i].ntag==ntag)
			{
				...
				{
					m_stock.black.packs[j].free   = INVALIED;
	...
	case MODE_FREE_COLOR://*************************
		//Search for the specified tag
		for(i=0;i<m_stock.fcolor.count;i++)
		{
			if(m_stock.fcolor.packs[i].ntag==ntag)
			{
				//Remove it
				m_stock.fcolor.totalh -= m_stock.fcolor.packs[i].h;
				//Close any gap
				for(BYTE j=i;j<m_stock.fcolor.count;j++)
				{
					m_stock.fcolor.packs[j].free   = VALIED;
					m_stock.fcolor.packs[j].type   = m_stock.fcolor.packs[j+1].type;
					m_stock.fcolor.packs[j].h	   = m_stock.fcolor.packs[j+1].h;
					m_stock.fcolor.packs[j].ntag   = m_stock.fcolor.packs[j+1].ntag;
					memcpy(m_stock.fcolor.packs[j].stag,m_stock.fcolor.packs[j+1].stag,4);
				}
				m_stock.fcolor.count--;
				break;
			}
...
CSpaceShipInfo	sinf;
	...
	m_stock.color.totalh = m_stock.fcolor.totalh = m_stock.black.totalh = m_stock.secure.totalh= 0;
	m_stock.color.count  = m_stock.fcolor.count = m_stock.black.count  = m_stock.secure.count = 0;
...
//--------------------------------------------------------------------------------------------
bool CMyStock::IsFreeColOver()
{
	if(m_stock.fcolor.totalh==0)
		return true;
	else
		return false;
}
...
MY_PACKS	*st = 0;
	...
	{//Has some colors
		...
		ditem->free	  = st->packs[highest].free;
...
MY_EX_REC ri[6];
	...
	//if((item[0].h==3)&&(item[0].type==CL_YELLOW))
		//int u=0;
			...
			for(i=0;i<m_cat.count;i++)
			{
				...
				{//Less than
					...
					{//Fresh one
						...
						{//Found match
							//Get pack details
							ri[index].item[ph_i].store = m_cat.item[i].store;
							ri[index].item[ph_i].type  = m_cat.item[i].type;
							ri[index].item[ph_i].ntag  = m_cat.item[i].ntag;
							ri[index].item[ph_i].h	   = m_cat.item[i].h;
							//Dirty this pack
...
BYTE least,loss,pat;
	...
	for(i=0;i<index;i++)
	{
		loss  = 0;
		//Cal droping marks
		for(BYTE c=0;c<ri[i].count;c++)
			loss += ri[i].item[c].h*ri[i].item[c].type;
		if(loss<least)
		{
			pat	  = i;
			least = loss;
	...
	if(pat!=200)
	{
		for(BYTE c=0;c<ri[pat].count;c++)
		{
			item[c+1].store = ri[pat].item[c].store;
			item[c+1].type  = ri[pat].item[c].type;
			item[c+1].ntag  = ri[pat].item[c].ntag;
			item[c+1].h     = ri[pat].item[c].h; 
		}
		*count = ri[pat].count+1;
		/*if(index>1)
			int p=0;*/
		return true;
	}
...
/*BYTE g[6];
	...
	{
		for(BYTE c=0;c<ri[i].count;c++)
			g[i] += ri[i].item[c].h*ri[i].item[c].type;
	}
...
BYTE CMyStock::GetStock(BYTE mode)
{
	...
	{
	case ST_COL_BK:
		return m_stock.color.totalh + m_stock.fcolor.totalh+ m_stock.black.totalh;
		break;
	case ST_SECURE:
...
{//Only call by  CMyExchange::ExchangePacks()
	for(BYTE i=0;i<m_stock.secure.count;i++)
	{
		...
		{
			//Make this pack as free
			m_map[m_stock.secure.packs[i].p.x][m_stock.secure.packs[i].p.y].free = VALIED;//To prevent helth decay
...
void CMyStock::RestoreSecurePacks(WORD ntag)
{
	for(BYTE i=0;i<m_stock.secure.count;i++)
	{
		if(m_stock.secure.packs[i].ntag==ntag)
		{//Re-store type
			m_map[m_stock.secure.packs[i].p.x][m_stock.secure.packs[i].p.y].free = INVALIED;
			break;
		}
	}
}

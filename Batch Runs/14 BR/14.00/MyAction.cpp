...
//-------------------------------------------------------------------------------------------------
void CMyAction::BlockArea(MY_POINT *loc)
{
int x,y,xs,ys;
...
BYTE		h;
	...
	for(BYTE i=0;i<25;i++)
	{
		obt = picture.GetObjectTypeAt(m_row_ord.p[i].x,m_row_ord.p[i].y);
		h	= picture.GetObjectHeightAt(m_row_ord.p[i].x,m_row_ord.p[i].y);

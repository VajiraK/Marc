...
//CORDINATE MAPPINGS ARE TRICKY ALTER CAREFULLY
	//Right
	p.x = 4;p.y = 4;
	if(PictureToMapEx(&p,&pr)==false)
		return	RET_ERROR;
	//Left
	p.x = 0;p.y = 4;
	if(PictureToMapEx(&p,&pl)==false)
		return	RET_ERROR;
...
BYTE  i=0;
	...
	{ 
		...
		y = m_gut_p.y;ys=m_gut_p.y+4;
			for(y=y;y<=ys;y++)
			{
				if((m_map[m_gut_p.x][y].type==CL_FREE)||(m_map[m_gut_p.x][y].type==CL_BLACK))
					i++;
				else
					break;
			}
		...
		x = m_gut_p.x;xs=m_gut_p.x+4;
			for(x=x;x<=xs;x++)
			{
				if((m_map[x][m_gut_p.y].type==CL_FREE)||(m_map[x][m_gut_p.y].type==CL_BLACK))
					i++;
				else
					break;
			}

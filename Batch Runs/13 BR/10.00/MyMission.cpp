...
static WORD st = 0;
	...
	DropBlackPacks(robot,GetColorHeight(),GetOposit(m_path[m_path_i-1].came_d));

	if(MakeWay(robot,GetOposit(m_facing),1,true,MODE_CHECK_HELTH)==RET_STOP_MIS)
		return RET_STOP_MIS;

	//Pick secure to dirty depth
	if(PickSecured(robot,false,GetFreeStepsEx(m_curent,m_facing,true))==RET_STOP_MIS)
		return RET_STOP_MIS;
...
int  bs,nt,nb;
	...
	for(int i=(m_path_i-1);i>=1;i--)
	{
		...
		if(IsBlackOver()==false)
			DropBlackPacks(robot,GetColorHeight(),GetOposit(m_facing));
...
bool bfound;
	...
	bst = IsBlackFeasible(chkdirty);
	if(bst==5)//Can't advance by removing blacks
		return RET_ERROR;

	for(BYTE i=0;i<3;i++)
	{
		if(PassBlacks(robot,drop,bst,mode,&bfound)==RET_STOP_MIS)
		{
		...
		if(tug.m_path[i].att==PA_TELE)
		{
	   		...
	   		if(tug.m_path[i].steps!=0)
			{
				...
				{//No space to go so PassBlacks
					...
					MakeWay(robot,GetOposit(m_facing),tug.m_path[i].steps,false,MODE_NO_CHECK_HELTH);
				}

				//These moves not save

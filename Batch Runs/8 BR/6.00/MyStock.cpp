...
BYTE		i;
	...
	pstk->item[i].realbk = item.realbk;
	pstk->item[i].type	= item.type;
	pstk->item[i].h		= item.h;
	pstk->item[i].ntag	= item.ntag;
	memcpy(pstk->item[i].stag,item.stag,4);
	
	//Point's all I need
	switch (item.type) 
	{ 
		case CL_YELLOW:
			pstk->item[i].points = item.h*20;
		break;
		case CL_PINK:
			pstk->item[i].points = item.h*15;
		break;
		case CL_BROWN:
			pstk->item[i].points = item.h*10;
		break;
		default:
...
BYTE	count = 0;
	...
	ditem->realbk = m_stock_cl.item[highest].realbk;
...
void CMyStock::ReleaseBlack(MY_ITEM *ditem)
{
	for(BYTE i=0;i<=ST_MAX;i++)
	{
		...
		{//Valied item
			//Return pakage details
			/*if(m_stock_bk.item[i].realbk==INVALIED)
				Beep(100,100);*/
			ditem->type	= m_stock_bk.item[i].type;
			ditem->realbk = m_stock_bk.item[i].realbk;

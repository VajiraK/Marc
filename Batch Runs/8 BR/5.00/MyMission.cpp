	...
	for(int i=(m_path_i-1);i>=1;i--)
	{
		...
		if(m_path[i].att==PA_TELE)
		{
			...
			{//No space to go so PassBlacks
				...
				if(i==(m_path_i-1))
					d = GetOposit(m_path[i+1].came_d);
				else
					d = m_path[i+1].came_d;
				//PassColors(robot,m_path[i+1].came_d);
   				PassBlacks(robot,d,MODE_NO_CHECK_HELTH);
			}
		...
		if(tug.m_path[i].att==PA_TELE)
		{
	   		...
	   		if(tug.m_path[i].steps!=0)
			{
				...
				{//No space to go so PassBlacks
					...
					if(PassBlacks(robot,GetOposit(m_facing),MODE_CHECK_HELTH)==RET_STOP_MIS)
					{//Duto low helth
						//Clip path
						m_path_i -= i;
						return RET_STOP_MIS;

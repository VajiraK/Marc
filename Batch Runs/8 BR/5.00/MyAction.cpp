...
{//Special DoMove (1)no save pos (2)batcheck to specified path location
	...
	{
		//Check battory
		if(CheckBatEx(upper,steps,GetStock(ST_ACTUAL))==RET_STOP_MIS)
			return RET_STOP_MIS;
		...
		{
		WORD bu;
			bu = 1 + ((10 + (GetStock(ST_ACTUAL) * 2))  *  steps);
			BatConsume(bu);
...
//-----------------------------------------------------------------------------------------------
BYTE CMyAction::DoMove(CRobot &robot,WORD steps,BYTE mode)
{//Commonly use DoMove over load
	if(steps!=0)
	{
		if(mode==MODE_MINING_PACKS)
		{
			...
			if(CheckBat(steps,GetStock(ST_ACTUAL))==RET_STOP_MIS)
				return RET_STOP_MIS;
		}

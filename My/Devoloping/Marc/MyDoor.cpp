...
BYTE CMyDoor::CrackDoor(CRobot &robot)
{
	...
	{//Facing a door
		m_door_p.x = 0;m_door_p.y = 4;
		//PictureToMapEx(&m_door_p,&m_door_p);
		if(PictureToMapEx(&m_door_p,&m_door_p)==false)
			return RET_ERROR;
				...
				{//Found
					...
					MY_POINT p; p.x = m_curent.x; p.y = m_curent.y;
						...
						if(lag<=steps)
						{
							...
							SetDirection(robot,FACE_DOWN);
							p.x = 0;p.y = 4;
							PictureToMapEx(&p,&m_door_p);//Re-map m_door_p(Because posisson is changed)
...
MY_POINT pic,map;;
	...
	{	
		...
		{
			if(m_map[map.x][map.y].type==CL_DOOR)
				return true;
		}
	}

...
int nb,bs,nt;
	...
	if(m_bat_boom.i!=0)
		nb = 1 + ((10+(stock*2)) * (bs-m_bat_boom.boom[GetUpperBoom(m_bat_boom.i)].st_srt)) + (nt*10) + bs + nt + m_bat_passblack + GetBoomLost(stock);
	else
		nb = 1 + ((10+(stock*2)) * bs) + (nt*10) + bs + nt + m_bat_passblack + GetBoomLost(stock);

	if((m_rem_bat)<=nb+(0.1*nb))
	{
		#ifdef _DEBUG
...
int bs,nt;
	...
	if(dh==0)//Zero height send when there is no room to secure even a one pack
		return;
	...
	{
		#ifdef _DEBUG
		if(bs<m_bat_boom.boom[GetUpperBoom(id)].st_srt)
			MessageBox(NULL,"Step topple! ","Marc",MB_OK|MB_ICONEXCLAMATION);
		#endif
		m_bat_boom.boom[id].st_nxt_b = bs - m_bat_boom.boom[GetUpperBoom(id)].st_srt;
	}else{
...
//------------------------------------------------------------------------------------------------
BYTE CMyBattery::GetUpperBoom(BYTE from)
{
int i;
	for(i=from-1;i>=0;i--)
	{
		if(m_bat_boom.boom[i].valied==VALIED)
			return i;//Get next valied
	}
	return 0;
}
